...
class QuotationsController extends AppController {
	...
	public function beforeFilter() {
		parent::beforeFilter();
		$this->Auth->allow('getQuotationData','getquotationproducts','getquotationinfo','getnewquotationcode','getquotationcurrencyid','getquotationiva','getquotationsforclient','generarOrdenDeVenta');		
	}
  ...
  public function getQuotationData(){
		...
		return json_encode($this->Quotation->getQuotationData($quotationId));
	}
  ...
  public function getquotationproducts() {
    ...
    define('DISPLAY_SALESORDER_ALL',0);
    define('DISPLAY_SALESORDER_PRESENT',1);
    define('DISPLAY_SALESORDER_ABSENT',2);
    $salesOrderDisplayOptions=[ 
      DISPLAY_SALESORDER_ALL => "Mostrar todas cotizaciones (con y sin orden de venta)",
      DISPLAY_SALESORDER_PRESENT => "Mostrar solamente cotizaciones con ordenes de venta",
      DISPLAY_SALESORDER_ABSENT => "Mostrar solamente cotizaciones sin ordenes de venta",
    ];
		$this->set(compact('salesOrderDisplayOptions'));
		
    define('DISPLAY_QUOTATION_ALL',0);
    define('DISPLAY_QUOTATION_ACTIVE',1);
    define('DISPLAY_QUOTATION_REJECTED',2);
		$quotationDisplayOptions=[
      DISPLAY_QUOTATION_ALL => "Mostrar todas cotizaciones (caídas y vigentes)",
      DISPLAY_QUOTATION_ACTIVE => "Mostrar solamente cotizaciones vigentes",
      DISPLAY_QUOTATION_REJECTED => "Mostrar solamente cotizaciones caídas",
    ];
		$this->set(compact('quotationDisplayOptions'));
      ...
      $userId=$loggedUserId;
		...
		if ($this->request->is('post')) {
			...
			$quotationDisplayOptionId=$this->request->data['Report']['quotation_display_option_id'];
			$salesOrderDisplayOptionId=$this->request->data['Report']['sales_order_display_option_id'];
		}		
		...
		$this->set(compact('quotationDisplayOptionId','salesOrderDisplayOptionId'));
    ...
    $userPendingCS=$userPendingUSD=$userPeriodCS=$userPeriodUSD=$userPeriod;
        ...
        if (array_key_exists($quotations[$i]['User']['id'],$userPeriod) && !$quotations[$i]['Quotation']['bool_rejected']){
        ...
        if (array_key_exists($pendingQuotations[$i]['User']['id'],$userPeriod) && !$pendingQuotations[$i]['Quotation']['bool_rejected'] ){
	...
	public function view($id = null) {
		...
		$fileName=str_replace('/','_',$quotation['Client']['name']).'_'.$quotation['Quotation']['quotation_code'];
    ...
    //  ],
    //];
    //echo 'Client id is '.$this->request->data['Quotation']['client_id'].'<br/>';
    $contactConditions=['Contact.client_id'=>$this->request->data['Quotation']['client_id']];
    //pr($contactConditions);
 ...
 */
    ...
    $canSeeAllUsers=$this->UserPageRight->hasUserPageRight('VER_LISTA_TODOS_USUARIOS',$userRoleId,$loggedUserId,'Quotations','verReporteCotizacionesPorEjecutivo');
    $this->set(compact('canSeeAllUsers'));
    
    $canSeeAllSalesExecutives=$this->UserPageRight->hasUserPageRight('VER_LISTA_TODOS_VENDEDORES',$userRoleId,$loggedUserId,'Quotations','verReporteCotizacionesPorEjecutivo');
    $this->set(compact('canSeeAllSalesExecutives'));
    //echo 'can see all sales executives:'.$canSeeAllSalesExecutives.'<br/>';
    ...
    if ($userRoleId == ROLE_ADMIN || $canSeeAllUsers || $canSeeAllSalesExecutives){
      ...
      $userId=$this->request->data['Report']['user_id'];
		...
		$this->set(compact('startDate','endDate','userId','currencyId','clientId'));
    ...
    if ($userId > 0){
      $clientUserConditions['user_id']=$userId;
    }
    ...
    for ($c=0;$c<count($allClients);$c++){
			if (empty($userId) || empty($allClients[$c]['ClientUser']) || $allClients[$c]['ClientUser'][0]['bool_assigned']){
        $thisClient=$allClients[$c];
        $thisClientId=$thisClient['Client']['id'];
        $clientIds[]=$thisClientId;        
      }
		}
    $clients=$this->Client->find('list',[
			'conditions'=>[
    ...
    $clientConditions=[];
    if ($clientId>0){
      $clientConditions[]=['Client.id'=>$clientId];
    }
		...
		for ($c=0;$c<count($selectedClients);$c++){
			...
			if ($userId > 0) { 
				$conditions['Quotation.user_id']=$userId;
			}
      //pr($conditions);
			$quotations=$this->Quotation->find('all',[
				'conditions'=>$conditions,
