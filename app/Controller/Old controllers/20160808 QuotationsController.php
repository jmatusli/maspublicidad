...
class QuotationsController extends AppController {
	...
	public function index() {
		...
		if ($userrole!=ROLE_ADMIN&&$userrole!=ROLE_ASSISTANT){
			$userConditions=array('User.id'=>$this->Auth->User('id'));
		}
	...
	public function add() {
		...
		$departments = $this->Product->Department->find('list',array('order'=>'Department.name'));
		...
		if ($this->request->is(array('post', 'put'))) {
			...
			for ($qp=0;$qp<count($this->request->data['QuotationProduct']);$qp++){
				$relatedProduct=$this->Product->find('first',array(
					'conditions'=>array(
						'Product.id'=>$this->request->data['QuotationProduct'][$qp]['product_id'],
					),
				));
				$this->request->data['QuotationProduct'][$qp]['bool_no_iva']=$relatedProduct['Product']['bool_no_iva'];
