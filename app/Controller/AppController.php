...
*/	
	...
	*/
	...
	public function repararArbolAcl() {
		...
		$this->set('title_for_layout', 'Reparar Árbol ACL');
		
		// 1. Verificar duplicados en lft/rght
		...
		// 2. Obtener todos los hijos de SalesOrders
		...
		foreach ($children as $index => $child) {
			$new_lft = $start_lft + ($index * 2);
			$new_rght = $new_lft + 1;
			
			$has_problem = false;
			if ($child['acos']['lft'] >= $child['acos']['rght']) {
				$problems[] = $child['acos']['alias'] . ': lft >= rght';
				$has_problem = true;
			}
			if ($child['acos']['lft'] <= 560 || $child['acos']['rght'] >= 617) {
				$problems[] = $child['acos']['alias'] . ': Fuera de rango';
				$has_problem = true;
			}
			
			if ($child['acos']['lft'] != $new_lft || $child['acos']['rght'] != $new_rght || $has_problem) {
				$corrections[] = array(
					'id' => $child['acos']['id'],
					'alias' => $child['acos']['alias'],
					'old_lft' => $child['acos']['lft'],
					'old_rght' => $child['acos']['rght'],
					'new_lft' => $new_lft,
					'new_rght' => $new_rght
				);
			}
		...
		// 4. Ejecutar reparación si se solicita
		if ($this->request->is('post') && !empty($this->request->data['execute_fix'])) {
			try {
				// Resetear todos los hijos de SalesOrders
				$db->query('UPDATE acos SET lft = 0, rght = 0 WHERE parent_id = 152');
				
				// Aplicar correcciones
				foreach ($corrections as $corr) {
					$db->query(
						"UPDATE acos SET lft = ?, rght = ? WHERE id = ?",
						array($corr['new_lft'], $corr['new_rght'], $corr['id'])
					);
				}
				
				// Actualizar rght de SalesOrders
				$new_salesorders_rght = $start_lft + (count($children) * 2);
				$db->query('UPDATE acos SET rght = ? WHERE id = 152', array($new_salesorders_rght));
				
				$this->Flash->success('Árbol ACL reparado exitosamente');
				$this->redirect(array('action' => 'repararArbolAcl'));
				
			} catch (Exception $e) {
				$this->Flash->error('Error al reparar el árbol ACL: ' . $e->getMessage());
			}
		}
		
		// 5. Verificar consistencia final
		$consistency = $db->fetchAll(
			'SELECT 
			...
			WHERE parent.alias = "SalesOrders"
			GROUP BY parent.id, parent.alias, parent.lft, parent.rght'
		);
		
		$this->set(compact('duplicates', 'children', 'problems', 'corrections', 'consistency'));
	}
