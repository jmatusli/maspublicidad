...
class RemissionsController extends AppController {
	...
	public function add() {
		...
		if ($this->request->is('post')) {
			...
			$previousRemissionsWithThisCode=$this->Remission->find('all',array(
				'conditions'=>array(
					...
					'Remission.client_id'=>$this->request->data['Remission']['client_id'],
				),
			));
			...
			elseif (empty($this->request->data['Remission']['client_id'])){
				...
				if ($this->request->data['Remission']['bool_annulled']){
					...
					try {
						...
						$client=$this->Client->find('first',array(
							'conditions'=>array(
								'Client.id'=>$this->request->data['Remission']['client_id'],
							),
						));
						//pr($client);
		...
		$clients = $this->Remission->Client->find('list');
 ...
 */
	public function delete($id = null) {
		...
		try {
			...
			$deletionArray['Deletion']['reference_id']=$remission['Remission']['id'];
			$deletionArray['Deletion']['reference']=$remission['Remission']['remission_code'];
			$deletionArray['Deletion']['type']='Remission';
