...
class SalesOrdersController extends AppController {
	...
	public function beforeFilter() {
		...
		// Permitir acceso a métodos AJAX sin autenticación
...
public function guardarReporteProduccionPendiente() {
	...
	public function view($id = null) {
	    ...
	    //	print_r(!$this->SalesOrder->exists($id));exit;
    ...
    //pr($salesOrder);
		...
		$bool_addsales_permission=$this->hasPermission($this->Session->read('User.id'),$aco_name);

		$this->set(compact('bool_addsales_permission'));
	...
	//$canDelProdNoFact=$this->UserPageRight->hasUserPageRight('ELIMINAR_PRODUCTO_NOFACT',$userRoleId,$loggedUserId,'SalesOrdersProducts','delete');

	// $this->set(compact('canDelProdNoFact'));
    ...
    $vendorUserId=0;
      ...
      $this->request->data['SalesOrder']['original_code' ]=$this->request->data['SalesOrder']['sales_order_code' ];
			...
			for ($sop=0;$sop<count($this->request->data['SalesOrderProduct']);$sop++){
	 			...
	 			if($this->request->data['SalesOrderProduct'][$sop]['sales_order_product_status_id']==2){
 ...
 */
	...
	/**/
		...
		// Manejar el parámetro de página
...
// 1. Buscamos qué ÓRDENES tienen al menos un producto de la categoría
...
// Configurar paginación para el reporte
...
$this->Paginator->settings = array(
    ...
    'order' => array('SalesOrder.sales_order_date' => 'ASC'),

    'limit' => 50, // 50 registros por página para mejor rendimiento

    'recursive' => 1,

    'page' => $page // Especificar la página actual
...
// 3. Reconstrucción para la vista

$salesOrders = array();

foreach ($productosPaginados as $p) {

    $nuevaFila = array();

    

    // Datos de la Orden: Fecha y Código

    $nuevaFila['SalesOrder'] = $p['SalesOrder'];

    // ID único (del producto) para evitar el colapso de filas en la vista

   // $nuevaFila['SalesOrder']['id'] = $p['SalesOrderProduct']['id']; 

    

    // Datos del Producto: Nombre y Cantidad
    ...
    // Lo guardamos como array de un solo elemento para el foreach de la vista
    ...
    $itemCat['name_category'] = $p['Product']['ProductCategory']['name'];

	

	

    $nuevaFila['ProductCategory'] = array($itemCat);



    $salesOrders[] = $nuevaFila;
...
$totalRecords = $this->params['paging']['SalesOrderProduct']['count'];

$totalPages = $this->params['paging']['SalesOrderProduct']['pageCount'];

$currentPage = $this->params['paging']['SalesOrderProduct']['page'];



// Pasar información de paginación a la vista

$this->set(compact('totalRecords', 'totalPages', 'currentPage'));
		...
		if (!empty($salesOrders)){
			...
			for ($so=0;$so<count($salesOrders);$so++){	
				...
				$salesOrders[$so]['SalesOrder']['order_status']=1;//$orderStatus;
		...
		//for ($s=0;$s<count($selectedStatuses);$s++){
			...
			$selectedStatuses[]['SalesOrders']=$salesOrdersOfStatus;			
		...
		$bool_complete_permission=$this->hasPermission($this->Session->read('User.id'),$aco_name);

		$this->set(compact('bool_complete_permission'));

		

		$this->set(compact(/*'annulledSalesOrders',*/'productCategories','product_category_id'));
	...
	// Endpoint dedicado SOLO para datos JSON del reporte

	public function getReporteCategoriasVentasData() {

		// FORZAR SALIDA JSON DIRECTA
		...
		try {
			...
			if ($this->request->is('post')) {
				...
				// Procesar datos del formulario correctamente
				if (isset($this->request->data['Report']['startdate'])) {

					$startData = $this->request->data['Report']['startdate'];

					$startDate = $startData['year'] . '-' . str_pad($startData['month'], 2, '0', STR_PAD_LEFT) . '-' . str_pad($startData['day'], 2, '0', STR_PAD_LEFT);

				}

				if (isset($this->request->data['Report']['enddate'])) {

					$endData = $this->request->data['Report']['enddate'];

					$endDate = $endData['year'] . '-' . str_pad($endData['month'], 2, '0', STR_PAD_LEFT) . '-' . str_pad($endData['day'], 2, '0', STR_PAD_LEFT);

				}

				if (isset($this->request->data['Report']['product_category_id'])) {
			...
			foreach ($productosPaginados as $p) {
				...
				$nuevaFila['Client'] = array('name' => $nombreCliente);
			...
			$response = array(
				...
				'pagination' => array(
					...
					'showingFrom' => (($currentPage - 1) * 50) + 1,

					'showingTo' => min($currentPage * 50, $totalRecords)
    ...
    public function guardarReporteCategoriasVentas() {

        $exportData=$_SESSION['reporteCategoriasVentas'];
