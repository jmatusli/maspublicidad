...
<?php
	$pageHeader="<thead>";
		$pageHeader.="<tr>";
			$pageHeader.="<th>".$this->Paginator->sort('movement_date')."</th>";
			$pageHeader.="<th>".$this->Paginator->sort('bool_input')."</th>";
			$pageHeader.="<th>".$this->Paginator->sort('entry_id')."</th>";
			$pageHeader.="<th>".$this->Paginator->sort('remission_id')."</th>";
			$pageHeader.="<th>".$this->Paginator->sort('stock_item_id')."</th>";
	...
	$excelHeader="<thead>";
		$excelHeader.="<tr>";
			$excelHeader.="<th>".$this->Paginator->sort('movement_date')."</th>";
			$excelHeader.="<th>".$this->Paginator->sort('bool_input')."</th>";
			$excelHeader.="<th>".$this->Paginator->sort('entry_id')."</th>";
			$excelHeader.="<th>".$this->Paginator->sort('remission_id')."</th>";
			$excelHeader.="<th>".$this->Paginator->sort('stock_item_id')."</th>";
	...
	foreach ($stockMovements as $stockMovement){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".h($stockMovement['StockMovement']['movement_date'])."</td>";
		$pageRow=""		$pageRow.="<td>".h($stockMovement['StockMovement']['bool_input'])."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($stockMovement['Entry']['id'], array('controller' => 'entries', 'action' => 'view', $stockMovement['Entry']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($stockMovement['Remission']['id'], array('controller' => 'remissions', 'action' => 'view', $stockMovement['Remission']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($stockMovement['StockItem']['name'], array('controller' => 'stock_items', 'action' => 'view', $stockMovement['StockItem']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($stockMovement['Product']['name'], array('controller' => 'products', 'action' => 'view', $stockMovement['Product']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($stockMovement['StockMovement']['product_quantity'])."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($stockMovement['MeasuringUnit']['name'], array('controller' => 'measuring_units', 'action' => 'view', $stockMovement['MeasuringUnit']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($stockMovement['StockMovement']['product_unit_cost'])."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($stockMovement['Currency']['abbreviation'], array('controller' => 'currencies', 'action' => 'view', $stockMovement['Currency']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($stockMovement['StockMovement']['description'])."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $stockMovement['StockMovement']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $stockMovement['StockMovement']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $stockMovement['StockMovement']['id']), array(), __('Are you sure you want to delete # %s?', $stockMovement['StockMovement']['id']));
			$pageRow.="</td>";
