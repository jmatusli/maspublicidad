<div class="inventoryProductInventoryProviders index">
<?php 
	echo "<h2>".__('Inventory Product Inventory Providers')."</h2>";
...
<?php
	$pageHeader="<thead>";
		$pageHeader.="<tr>";
			$pageHeader.="<th>".$this->Paginator->sort('inventory_product_id')."</th>";
	...
	$excelHeader="<thead>";
		$excelHeader.="<tr>";
			$excelHeader.="<th>".$this->Paginator->sort('inventory_product_id')."</th>";
	...
	foreach ($inventoryProductInventoryProviders as $inventoryProductInventoryProvider){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".$this->Html->link($inventoryProductInventoryProvider['InventoryProduct']['name'], array('controller' => 'inventory_products', 'action' => 'view', $inventoryProductInventoryProvider['InventoryProduct']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($inventoryProductInventoryProvider['InventoryProvider']['name'], array('controller' => 'inventory_providers', 'action' => 'view', $inventoryProductInventoryProvider['InventoryProvider']['id']))."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $inventoryProductInventoryProvider['InventoryProductInventoryProvider']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $inventoryProductInventoryProvider['InventoryProductInventoryProvider']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $inventoryProductInventoryProvider['InventoryProductInventoryProvider']['id']), array(), __('Are you sure you want to delete # %s?', $inventoryProductInventoryProvider['InventoryProductInventoryProvider']['id']));
			$pageRow.="</td>";
