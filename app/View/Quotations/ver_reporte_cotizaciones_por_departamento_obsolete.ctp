...
<?php	
	echo "<h2>".__('Reporte de Cotizaciones por Departamento y Producto')."</h2>";
	...
	echo $this->Form->create('Report'); 
		echo "<fieldset>";
			...
			echo "<br/>";			
			echo $this->Form->input('Report.department_id',array('label'=>__('Department'),'default'=>'0','empty'=>array('0'=>__('Seleccione Departamento'))));
	...
	echo $this->Html->link(__('Guardar como Excel'), array('action' => 'guardarReporteCotizacionesPorDepartamento'), array( 'class' => 'btn btn-primary')); 
	...
	foreach ($selectedDepartments as $department){
		//pr($department);
		if (!empty($department['QuotationProducts'])){
			$title="Cotizaciones para Departamento ".$this->Html->Link($department['Department']['name'],array('controller'=>'departments','action'=>'view',$department['Department']['id']));
			...
			$bodyRows="";
			foreach ($department['QuotationProducts'] as $product){
				//pr($product);
			...
			$table_id=substr("departamento_".trim($department['Department']['name']),0,30);
			echo "<table id='".$table_id."'>".$outputhead.$body."</table>";
			$excel.="<table id='".$table_id."'>".$excelhead.$body."</table>";
		}
	}
	$_SESSION['reporteCotizacionesPorDepartamento'] = $excel;
?>
</div>
<script>
	$('body').on('change','#ReportDepartmentId',function(){	
		var departmentid=$(this).children("option").filter(":selected").val();
		$.ajax({
			url: '<?php echo $this->Html->url('/'); ?>products/getproductsfordepartment/',
