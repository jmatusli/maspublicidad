<script>
	...
	$('body').on('change','.productid',function(){
		setMeasuringUnit($(this).find('div select').val(),$(this).closest('tr').attr('row'));
	});	
	...
	function calculateTotal(){
		...
		$('#EntryCostSubtotal').val(roundToTwo(subtotalCost));
		$('tr.totalrow.subtotal td.totalcost span.amount').text(roundToTwo(subtotalCost));
		...
		$('#EntryCostTotal').val(roundToTwo(totalCost));
		$('tr.totalrow.total td.totalcost span.amount').text(roundToTwo(totalCost));
	...
	$(document).ready(function(){
		...
		$('#productsForEntry tr').each(function(){
			...
			var currentrow=$('#productsForEntry').find("[row='" + rowid + "']");
			var productid=currentrow.find('.productid div select').val();
			if (productid>0){
...
<?php 
	...
	echo "<fieldset>"; 
		echo "<legend>".__('Add Entry')."</legend>";
		echo "<div class='container-fluid'>";
			echo "<div class='rows'>";
				echo "<div class='col-md-9'>";
					echo $this->Form->input('entry_date',array('dateFormat'=>'DMY'));
					echo $this->Form->input('exchange_rate',array('default'=>$exchangeRateEntry,'readonly'=>'readonly'));
					...
					echo $this->Form->input('bool_iva',array('checked'=>false,'div'=>array('class'=>'hidden')));
				...
				echo "<div class='col-md-3'>";					
					...
					echo $this->Form->input('cost_iva',array('readonly'=>'readonly','type'=>'decimal','default'=>'0', 'between' => '<span class=\'currency totalbox\'></span>','div'=>array('class'=>'hidden')));
					echo $this->Form->input('cost_total',array('readonly'=>'readonly','type'=>'decimal','default'=>'0', 'between' => '<span class=\'currency totalbox\'></span>','div'=>array('class'=>'hidden')));
				echo "</div>";
					...
					echo "<table id='productsForEntry'>";
						echo "<thead>";
							echo "<tr>";
								...
								echo "<th>".__('Costo Unitario')."</th>";
								echo "<th>".__('Costo Total')."</th>";
								echo "<th></th>";
							echo "</tr>";
						echo "</thead>";
						echo "<tbody>";
						for ($i=0;$i<25;$i++) { 
							...
							echo "<tr class='totalrow subtotal'>";
								...
								echo "<td></td>";
							echo "</tr>";		
							echo "<tr class='totalrow iva hidden'>";
								echo "<td>IVA</td>";
								echo "<td></td>";
								echo "<td></td>";
								echo "<td class='totalcost amount right'><span class='currency'></span><span class='amount'>0</span></td>";
								echo "<td></td>";
							echo "</tr>";		
							echo "<tr class='totalrow total hidden'>";
								echo "<td>Total</td>";
								echo "<td></td>";
								echo "<td></td>";
								echo "<td class='totalcost amount right'><span class='currency'></span><span class='amount'>0</span></td>";
