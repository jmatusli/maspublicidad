...
<?php 
	...
	echo "<dl>";
		...
		echo "<dd>".$this->Html->link($entry['InventoryProvider']['name'], array('controller' => 'inventory_providers', 'action' => 'view', $entry['InventoryProvider']['id']))."</dd>";
	...
	if (!empty($entry['StockMovement'])){
		...
		foreach ($entry['StockMovement'] as $stockMovement){ 
			echo "<tr>";
				...
				echo "<td>".$stockMovement['product_quantity']." ".$stockMovement['MeasuringUnit']['abbreviation']."</td>";
...
<?php 
	...
	echo "<ul>";
		...
		if ($bool_delete_permission){
			echo "<li>".$this->Form->postLink(__('Delete Entry'), array('action' => 'delete', $entry['Entry']['id']), array(), __('Está seguro que quiere eliminar entrada #%s?', $entry['Entry']['entry_code']))."</li>";
			echo "<br/>";
		}
