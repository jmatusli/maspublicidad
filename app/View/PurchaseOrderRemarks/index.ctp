<div class="purchaseOrderRemarks index">
<?php 
	echo "<h2>".__('Purchase Order Remarks')."</h2>";
...
<?php
	...
	foreach ($purchaseOrderRemarks as $purchaseOrderRemark){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".$this->Html->link($purchaseOrderRemark['User']['username'], array('controller' => 'users', 'action' => 'view', $purchaseOrderRemark['User']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($purchaseOrderRemark['PurchaseOrder']['id'], array('controller' => 'purchase_orders', 'action' => 'view', $purchaseOrderRemark['PurchaseOrder']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($purchaseOrderRemark['PurchaseOrderRemark']['remark_datetime'])."</td>";
		$pageRow=""		$pageRow.="<td>".h($purchaseOrderRemark['PurchaseOrderRemark']['remark_text'])."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($purchaseOrderRemark['ActionType']['name'], array('controller' => 'action_types', 'action' => 'view', $purchaseOrderRemark['ActionType']['id']))."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $purchaseOrderRemark['PurchaseOrderRemark']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $purchaseOrderRemark['PurchaseOrderRemark']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $purchaseOrderRemark['PurchaseOrderRemark']['id']), array(), __('Are you sure you want to delete # %s?', $purchaseOrderRemark['PurchaseOrderRemark']['id']));
			$pageRow.="</td>";
