<div class="productionProcessRemarks index">
<?php 
	echo "<h2>".__('Production Process Remarks')."</h2>";
...
<?php
	...
	foreach ($productionProcessRemarks as $productionProcessRemark){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".$this->Html->link($productionProcessRemark['User']['username'], array('controller' => 'users', 'action' => 'view', $productionProcessRemark['User']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($productionProcessRemark['ProductionProcess']['id'], array('controller' => 'production_processes', 'action' => 'view', $productionProcessRemark['ProductionProcess']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($productionProcessRemark['ProductionProcessRemark']['remark_datetime'])."</td>";
		$pageRow=""		$pageRow.="<td>".h($productionProcessRemark['ProductionProcessRemark']['remark_text'])."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($productionProcessRemark['ActionType']['name'], array('controller' => 'action_types', 'action' => 'view', $productionProcessRemark['ActionType']['id']))."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $productionProcessRemark['ProductionProcessRemark']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $productionProcessRemark['ProductionProcessRemark']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $productionProcessRemark['ProductionProcessRemark']['id']), array(), __('Are you sure you want to delete # %s?', $productionProcessRemark['ProductionProcessRemark']['id']));
			$pageRow.="</td>";
