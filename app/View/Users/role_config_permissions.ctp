...
<?php 
	...
	echo "<fieldset>";
		echo '<legend>Permisos de Configuración</legend>';
				...
				for ($a=0;$a<count($selectedControllers[$c]['actions']);$a++){
					...
					echo "<tr>";
						...
						for ($r=0;$r<count($selectedControllers[$c]['actions'][$a]['rolePermissions']);$r++){
							...
							echo "<td>".$this->Form->input('Role.'.$r.'.Controller.'.$c.'.Action.'.$a,array('type'=>'checkbox','label'=>false,'checked'=>$selectedControllers[$c]['actions'][$a]['rolePermissions'][$r]))."</td>";;
						}
					echo "</tr>";
				}
			}
