<div class="productionOrderProductDepartments index">
<?php 
	echo "<h2>".__('Production Order Product Departments')."</h2>";
...
<?php
	$pageHeader="<thead>";
		$pageHeader.="<tr>";
			...
			$pageHeader.="<th>".$this->Paginator->sort('rank')."</th>";
	...
	$excelHeader="<thead>";
		$excelHeader.="<tr>";
			...
			$excelHeader.="<th>".$this->Paginator->sort('rank')."</th>";
		$excelHeader.="</tr>";
	$excelHeader.="</thead>";
	...
	foreach ($productionOrderProductDepartments as $productionOrderProductDepartment){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".$this->Html->link($productionOrderProductDepartment['ProductionOrderProduct']['id'], array('controller' => 'production_order_products', 'action' => 'view', $productionOrderProductDepartment['ProductionOrderProduct']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($productionOrderProductDepartment['Department']['name'], array('controller' => 'departments', 'action' => 'view', $productionOrderProductDepartment['Department']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($productionOrderProductDepartment['ProductionOrderProductDepartment']['rank'])."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $productionOrderProductDepartment['ProductionOrderProductDepartment']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $productionOrderProductDepartment['ProductionOrderProductDepartment']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $productionOrderProductDepartment['ProductionOrderProductDepartment']['id']), array(), __('Are you sure you want to delete # %s?', $productionOrderProductDepartment['ProductionOrderProductDepartment']['id']));
			$pageRow.="</td>";
