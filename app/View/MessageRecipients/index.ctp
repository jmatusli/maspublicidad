...
<?php 
	echo "<h2>".__('Message Recipients')."</h2>";
...
<?php
	$pageHeader="<thead>";
		$pageHeader.="<tr>";
			$pageHeader.="<th>".$this->Paginator->sort('message_id')."</th>";
			$pageHeader.="<th>".$this->Paginator->sort('recipient_user_id')."</th>";
			$pageHeader.="<th>".$this->Paginator->sort('bool_read')."</th>";
			$pageHeader.="<th class='actions'>".__('Actions')."</th>";
		$pageHeader.="</tr>";
	$pageHeader.="</thead>";
	$excelHeader="<thead>";
		$excelHeader.="<tr>";
			$excelHeader.="<th>".$this->Paginator->sort('message_id')."</th>";
			$excelHeader.="<th>".$this->Paginator->sort('recipient_user_id')."</th>";
			$excelHeader.="<th>".$this->Paginator->sort('bool_read')."</th>";
		$excelHeader.="</tr>";
	$excelHeader.="</thead>";
	...
	foreach ($messageRecipients as $messageRecipient){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".$this->Html->link($messageRecipient['Message']['id'], array('controller' => 'messages', 'action' => 'view', $messageRecipient['Message']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($messageRecipient['RecipientUser']['username'], array('controller' => 'users', 'action' => 'view', $messageRecipient['RecipientUser']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($messageRecipient['MessageRecipient']['bool_read'])."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $messageRecipient['MessageRecipient']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $messageRecipient['MessageRecipient']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $messageRecipient['MessageRecipient']['id']), array(), __('Are you sure you want to delete # %s?', $messageRecipient['MessageRecipient']['id']));
			$pageRow.="</td>";
