...
<?php 
	...
	echo "<fieldset>";
		...
		echo "<div class='container-fluid'>";
			echo "<div class='rows'>";
				echo "<div class='col-md-10'>";	
					echo $this->Form->input('Reassign.origin_user_id',array('label'=>__('Usuario de Origen de quien se van a transferir los Clientes'),'default'=>$originUserId,'empty'=>array('0'=>__('Seleccione un Usuario'))));
					echo $this->Form->submit('Mostrar Clientes para Vendedor de Origen',['id'=>'showclients','name'=>'showclients','style'=>'width:400px;']); 					
					...
					echo $this->Form->input('Reassign.bool_keep_origin',array('label'=>__('Mantener Vendedor de Origen'),'default'=>$boolKeepOrigin,'options'=>$keepArray));
					...
					//if ($userrole==ROLE_ADMIN||$userrole==ROLE_ASSISTANT) { 
						echo $this->Form->input('Reassign.destiny_user_id',array('label'=>__('Ejecutivo(s) de venta a quien se van a transferir los Clientes'),'default'=>$destinyUserArray,'empty'=>array('0'=>__('Seleccione un Usuario')),'multiple'=>true,'lines'=>5));
					//}					
				echo "</div>";
			...
			echo "<div class='rows'>";	
				...
				if (empty($originUserId)){
					...
					if (empty($clientsAssociatedWithUser)){
						...
						echo $this->Form->input('Reassign.powerselector1',array('class'=>'powerselector','checked'=>true,'style'=>'width:5em;','label'=>array('text'=>'Seleccionar/Deseleccionar todos clientes','style'=>'padding-left:5em;'),'format' => array('before', 'input', 'between', 'label', 'after', 'error' )));
						...
						echo "<table>";
							...
							foreach ($clientsAssociatedWithUser as $client){
								echo "<tr>";
									echo "<td class='selector'>".$this->Form->input('Reassign.Client.'.$client['Client']['id'].'.selector',array('checked'=>true,'label'=>false))."</td>";
									echo "<td>".$this->Html->link($client['Client']['name'],array('controller'=>'clients','action'=>'view',$client['Client']['id']))."</td>";
									echo "<td>".$this->Form->input('Reassign.Client.'.$client['Client']['id'].'.target_user_id',array('label'=>false,'default'=>$destinyUserArray,'class'=>'targetuser','empty'=>array('0'=>__('Usuario(s) de Destino')),'multiple'=>true,'lines'=>5))."</td>";
								echo "</tr>";
							}
							echo "</tbody>";
						echo "</table>";
						echo $this->Form->input('Reassign.powerselector2',array('class'=>'powerselector','checked'=>true,'style'=>'width:5em;','label'=>array('text'=>'Seleccionar/Deseleccionar todos clientes','style'=>'padding-left:5em;'),'format' => array('before', 'input', 'between', 'label', 'after', 'error' )));
					}
				}
				echo "</div>";
			echo "</div>";
		echo "</div>";		
	echo "</fieldset>";
	echo $this->Form->submit('Reasignar Clientes',array('id'=>'reassign','name'=>'reassign','style'=>'width:400px;')); 
	echo $this->Form->end(); 
?>
</div>
