<div class="salesOrderProductDepartments index">
<?php 
	echo "<h2>".__('Sales Order Product Departments')."</h2>";
...
<?php
	$pageHeader="<thead>";
		$pageHeader.="<tr>";
			$pageHeader.="<th>".$this->Paginator->sort('sales_order_product_id')."</th>";
	...
	$excelHeader="<thead>";
		$excelHeader.="<tr>";
			$excelHeader.="<th>".$this->Paginator->sort('sales_order_product_id')."</th>";
	...
	foreach ($salesOrderProductDepartments as $salesOrderProductDepartment){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".$this->Html->link($salesOrderProductDepartment['SalesOrderProduct']['id'], array('controller' => 'sales_order_products', 'action' => 'view', $salesOrderProductDepartment['SalesOrderProduct']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($salesOrderProductDepartment['Department']['name'], array('controller' => 'departments', 'action' => 'view', $salesOrderProductDepartment['Department']['id']))."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $salesOrderProductDepartment['SalesOrderProductDepartment']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $salesOrderProductDepartment['SalesOrderProductDepartment']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $salesOrderProductDepartment['SalesOrderProductDepartment']['id']), array(), __('Are you sure you want to delete # %s?', $salesOrderProductDepartment['SalesOrderProductDepartment']['id']));
			$pageRow.="</td>";
