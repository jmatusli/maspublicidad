...
<?php 
	echo "<h2>".__('Inventory Providers')."</h2>";
...
<?php
	...
	foreach ($inventoryProviders as $inventoryProvider){ 
		$pageRow="";
			...
			$pageRow.="<td>".h($inventoryProvider['InventoryProvider']['name']).($inventoryProvider['InventoryProvider']['bool_active']?"":" (Inactivo)")."</td>";
		
			$pageRow.="<td>".h($inventoryProvider['InventoryProvider']['address'])."</td>";
			$pageRow.="<td>".h($inventoryProvider['InventoryProvider']['phone'])."</td>";
			$pageRow.="<td>".$this->Text->autoLinkEmails($inventoryProvider['InventoryProvider']['email'])."</td>";

			if ($inventoryProvider['InventoryProvider']['bool_active']){
			...
			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $inventoryProvider['InventoryProvider']['id']));
				if ($bool_edit_permission){
					$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $inventoryProvider['InventoryProvider']['id']));
				}
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $inventoryProvider['InventoryProvider']['id']), array(), __('Are you sure you want to delete # %s?', $inventoryProvider['InventoryProvider']['id']));
			$pageRow.="</td>";
		if ($inventoryProvider['InventoryProvider']['bool_active']){
