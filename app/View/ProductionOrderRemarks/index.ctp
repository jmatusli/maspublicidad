<div class="productionOrderRemarks index">
<?php 
	echo "<h2>".__('Production Order Remarks')."</h2>";
...
<?php
	$pageHeader="<thead>";
		$pageHeader.="<tr>";
			...
			$pageHeader.="<th>".$this->Paginator->sort('production_order_id')."</th>";
	...
	$excelHeader="<thead>";
		$excelHeader.="<tr>";
			...
			$excelHeader.="<th>".$this->Paginator->sort('production_order_id')."</th>";
	...
	foreach ($productionOrderRemarks as $productionOrderRemark){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".$this->Html->link($productionOrderRemark['User']['username'], array('controller' => 'users', 'action' => 'view', $productionOrderRemark['User']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($productionOrderRemark['ProductionOrder']['id'], array('controller' => 'production_orders', 'action' => 'view', $productionOrderRemark['ProductionOrder']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($productionOrderRemark['ProductionOrderRemark']['remark_datetime'])."</td>";
		$pageRow=""		$pageRow.="<td>".h($productionOrderRemark['ProductionOrderRemark']['remark_text'])."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $productionOrderRemark['ProductionOrderRemark']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $productionOrderRemark['ProductionOrderRemark']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $productionOrderRemark['ProductionOrderRemark']['id']), array(), __('Are you sure you want to delete # %s?', $productionOrderRemark['ProductionOrderRemark']['id']));
			$pageRow.="</td>";
