...
<?php
	...
	$output.="<table style='width:100%'>";
		$output.="<tr>";
			$output.="<td style='width:70%'>";
				$output.="<div>Proveedor:<span class='underline'>".$entry['Provider']['name']."</span></div>";
			$output.="</td>";
	...
	if (!empty($entry['StockMovement'])){
		...
		foreach ($entry['StockMovement'] as $stockMovement){ 
			$output.="<tr>";
				$output.="<td>".$stockMovement['product_id']."</td>";
				$output.="<td>".$stockMovement['product_quantity']."</td>";
