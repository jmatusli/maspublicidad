<script>
  ...
  $('body').on('change','#SalesOrderBoolAuthorized',function(){	
		if ($(this).is(':checked')){
			$('#SalesOrderAuthorizingUserId').val('<?php echo $loggedUserId; ?>');
  ...
  function updateQuotationData(quotationId){
		$.ajax({
			url: '<?php echo $this->Html->url('/'); ?>quotations/getQuotationData/',
			data:{"quotationId":quotationId},
			cache: false,
			type: 'POST',
      dataType: "json",
			success: function (quotation) {
        $('#SalesOrderClientId').val(quotation.Quotation.client_id);
        $('#SalesOrderCurrencyId').val(quotation['Quotation']['currency_id']);
        $('#SalesOrderVendorUserId').val(quotation['Quotation']['user_id']);
        $('#SalesOrderBoolIva').prop('checked',quotation['Quotation']['bool_iva']);
				updateCurrencies();
			},
			error: function(e){
				alert(e.responseText);
...
*/  
  ...
  $('body').on('change','#SalesOrderRemarkRemarkText',function(){
    addEditionNumberToSalesOrderCode();
  });
  ...
  $('body').on('change','.productdescription',function(){	
		calculateRow($(this).closest('tr').attr('row'));
		calculateTotal();
    addEditionNumberToSalesOrderCode();
	});	
  ...
  function addEditionNumberToSalesOrderCode(){
    var currentSalesOrderCode=$('#SalesOrderSalesOrderCode').val();
    var originalSalesOrderCode=$('#SalesOrderOriginalCode').val();
    if (originalSalesOrderCode == currentSalesOrderCode){
      var newSalesOrderCode='';
      
      var newEditionEnding="_E1";
      var finalComponentCode=currentSalesOrderCode.substr(currentSalesOrderCode.lastIndexOf("_")+1);
      if (finalComponentCode.startsWith("E")){
        var editionNumber=parseInt(currentSalesOrderCode.substr(currentSalesOrderCode.lastIndexOf("_")+2));
        ...
        newSalesOrderCode=currentSalesOrderCode.substr(0,currentSalesOrderCode.lastIndexOf("_"))+newEditionEnding
      }
      else {
        newSalesOrderCode=currentSalesOrderCode+newEditionEnding
      }
      $('#SalesOrderSalesOrderCode').val(newSalesOrderCode);
      $('#titleCode').html(newSalesOrderCode);
...
<?php 
	...
	echo "<fieldset>"; 
		echo '<legend>'.__('Edit Sales Order').' <span id="titleCode">'.$this->request->data['SalesOrder']['sales_order_code'].'</span></legend>'; 
    //pr($this->request->data['SalesOrder']);
		echo '<div class="container-fluid">';
			echo "<div class='rows'>";	
				echo "<div class='col-md-4'>";					
					echo $this->Form->input('id');
					echo $this->Form->input('SalesOrder.quotation_id',['default'=>$this->request->data['SalesOrder']['quotation_id'],'empty'=>['0'=>'Seleccione Cotización']]);
          echo $this->Form->input('client_id',['type'=>'hidden','value'=>$clientId]);
          echo $this->Form->input('vendor_user_id',['type'=>'hidden','value'=>$vendorUserId]);
					...
					echo $this->Form->input('sales_order_code',['readonly'=>'readonly']);
          echo $this->Form->input('original_code',['type'=>'hidden']);
				...
				echo "<div class='col-md-5'>";
					...
					echo $this->Form->input('SalesOrderRemark.remark_text',['rows'=>2]);
				...
				echo "<div class='col-md-3'>";
					...
					echo "<div class='topright quotation'>";
						...
						echo "<h3>Resumen Cotización ".$this->Html->link($relatedQuotation['Quotation']['quotation_code'],['controller'=>'quotations','action'=>'view',$relatedQuotation['Quotation']['id']])."</h3>";
						echo "<dl>";
