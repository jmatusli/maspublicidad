<div class="invoiceRemarks index">
<?php 
	echo "<h2>".__('Invoice Remarks')."</h2>";
...
<?php
	...
	foreach ($invoiceRemarks as $invoiceRemark){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".$this->Html->link($invoiceRemark['User']['username'], array('controller' => 'users', 'action' => 'view', $invoiceRemark['User']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($invoiceRemark['Invoice']['invoice_code'], array('controller' => 'invoices', 'action' => 'view', $invoiceRemark['Invoice']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($invoiceRemark['InvoiceRemark']['remark_datetime'])."</td>";
		$pageRow=""		$pageRow.="<td>".h($invoiceRemark['InvoiceRemark']['remark_text'])."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($invoiceRemark['ActionType']['name'], array('controller' => 'action_types', 'action' => 'view', $invoiceRemark['ActionType']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($invoiceRemark['InvoiceRemark']['working_days_before_reminder'])."</td>";
		$pageRow=""		$pageRow.="<td>".h($invoiceRemark['InvoiceRemark']['reminder_date'])."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $invoiceRemark['InvoiceRemark']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $invoiceRemark['InvoiceRemark']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $invoiceRemark['InvoiceRemark']['id']), array(), __('Are you sure you want to delete # %s?', $invoiceRemark['InvoiceRemark']['id']));
			$pageRow.="</td>";
