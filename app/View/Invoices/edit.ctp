	...
	*/
	...
	function loadProductsForSalesOrder(selectedSalesOrders){
		...
		$.ajax({
			...
			data:{"selectedSalesOrders":selectedSalesOrders,"boolIVA":boolIVA,"boolskipproductchecks":boolskipproductchecks,"currencyid":currencyid,"exchangerate":exchangerate,"invoiceid":invoiceid},
			cache: false,
			type: 'POST',
...
<?php 
	...
	echo "<fieldset>";
		...
		echo $this->Form->input('bool_skip_product_checks',array('label'=>'Control de estado de productos', 'options'=>$productChecks,'onclick'=>'return false')); 
		echo $this->Form->submit(__('Refresh'),array('name'=>'refresh', 'id'=>'refresh')); 
    ...
    $productTableRows='';
        ...
        $productTableRows.='<td class="productid">';
          ...
          $productTableRows.=$this->Form->input('InvoiceProduct.'.$i.'.sales_order_product_id',['label'=>false,'type'=>'hidden','value'=>$productsForInvoice[$i]['id']]);
        $productTableRows.='</td>';
        ...
        $productTableRows.='<td class="pendingquantity amount">'.$this->Form->input('InvoiceProduct.'.$i.'.pending_quantity',['label'=>false,'value'=>$productsForInvoice[$i]['product_quantity'],'type'=>'numeric','readonly'=>true]).'</td>';
	...
	echo $this->Form->submit(__('Submit'),array('name'=>'submit', 'id'=>'submit','div'=>array('class'=>'submit submission'))); 
	echo $this->Form->submit(__('Guardar y siguiente Factura'),array('name'=>'saveandnext', 'id'=>'saveandnext','div'=>array('class'=>'submit saveandnext'))); 
	echo $this->Form->end(); 
?>
</div>
<?php
