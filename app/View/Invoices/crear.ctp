<script>
    ...
    loadClientData(clientid);
		...
		var invoiceid=0;
		$.ajax({
			url: '<?php echo $this->Html->url('/'); ?>salesOrders/getSalesOrdersForClient/',
  ...
  var jsClientsBySalesOrder=<?php echo json_encode($clientsBySalesOrder); ?>;
	function loadSalesOrder(){
		var salesorderid=$('#InvoiceSalesOrderId').children("option").filter(":selected").val();
    
    // 20211018 WHILE THIS CODE WORKS, THE SALES ORDERS ONLY BECOME AVAILABLE AFTER SELECTING THE CLIENT, THUS THE CLIENT DATA ARE ALREADY LOADED
    //var clientId=( jsClientsBySalesOrder[salesorderid] === undefined ? 0 : jsClientsBySalesOrder[salesorderid]);
    //loadClientData(clientId);
	...
	echo '<fieldset>';
		...
		echo '<div class="container-fluid">';
          ...
          echo $this->Form->input('reference',['rows'=>2]);
				...
				echo '<div class="col-sm-4">';
          echo '<h2>Cliente</h2>';
          echo $this->Form->input('client_id',['label'=>'Cliente registrado','default'=>'0','empty'=>['0'=>'-- Cliente --']]);
...
<div class="actions">
	...
	<ul>
		...
		<li><?php echo $this->Html->link(__('List Invoices'), ['action' => 'resumen']); ?></li>
		<br/>
	<?php
