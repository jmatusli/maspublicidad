<div class="productionOrderProductOperationLocations index">
<?php 
	echo "<h2>".__('Production Order Product Operation Locations')."</h2>";
...
<?php
	$pageHeader="<thead>";
		$pageHeader.="<tr>";
			$pageHeader.="<th>".$this->Paginator->sort('production_order_product_id')."</th>";
	...
	$excelHeader="<thead>";
		$excelHeader.="<tr>";
			$excelHeader.="<th>".$this->Paginator->sort('production_order_product_id')."</th>";
	...
	foreach ($productionOrderProductOperationLocations as $productionOrderProductOperationLocation){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".$this->Html->link($productionOrderProductOperationLocation['ProductionOrderProduct']['id'], array('controller' => 'production_order_products', 'action' => 'view', $productionOrderProductOperationLocation['ProductionOrderProduct']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($productionOrderProductOperationLocation['OperationLocation']['name'], array('controller' => 'operation_locations', 'action' => 'view', $productionOrderProductOperationLocation['OperationLocation']['id']))."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $productionOrderProductOperationLocation['ProductionOrderProductOperationLocation']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $productionOrderProductOperationLocation['ProductionOrderProductOperationLocation']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $productionOrderProductOperationLocation['ProductionOrderProductOperationLocation']['id']), array(), __('Are you sure you want to delete # %s?', $productionOrderProductOperationLocation['ProductionOrderProductOperationLocation']['id']));
			$pageRow.="</td>";
