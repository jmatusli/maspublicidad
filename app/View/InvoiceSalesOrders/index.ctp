<div class="invoiceSalesOrders index">
<?php 
	echo "<h2>".__('Invoice Sales Orders')."</h2>";
...
<?php
	...
	foreach ($invoiceSalesOrders as $invoiceSalesOrder){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".$this->Html->link($invoiceSalesOrder['Invoice']['invoice_code'], array('controller' => 'invoices', 'action' => 'view', $invoiceSalesOrder['Invoice']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($invoiceSalesOrder['SalesOrder']['sales_order_code'], array('controller' => 'sales_orders', 'action' => 'view', $invoiceSalesOrder['SalesOrder']['id']))."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $invoiceSalesOrder['InvoiceSalesOrder']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $invoiceSalesOrder['InvoiceSalesOrder']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $invoiceSalesOrder['InvoiceSalesOrder']['id']), array(), __('Are you sure you want to delete # %s?', $invoiceSalesOrder['InvoiceSalesOrder']['id']));
			$pageRow.="</td>";
