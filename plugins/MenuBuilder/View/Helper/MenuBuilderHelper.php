 ...
 */
	...
	protected function _buildItem(&$item, $pos = -1, &$isActive = false, $options=array()) {
		...
		//pr($this->_group);
		if ($item['separator']) {
			//echo "threw the separator";
			return $item['separator'];
		}

		if ($item['title'] === null) {
			//echo "threw null title";
			return '';
		}

		if (!empty($item['permissions']) && !in_array($this->_group, (array)$item['permissions'])) {
			//echo "threw null permissions";
			return '';
		}
		$children = '';
		//echo "processing the item";
		$nowIsActive = false;
